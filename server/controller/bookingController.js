...
exports.bookEvent = async (req, res) => {
    try {
        ...
        if (existingBooking && existingBooking.status !== 'cancelled') {
            return res.status(400).json({ message: 'Already booked or pending for this event' });
        }
...
exports.confirmBooking = async (req, res) => {
    try {
       ...
       const booking = await Booking.findById(req.params.id).populate('userId', '-password').populate('eventId');
